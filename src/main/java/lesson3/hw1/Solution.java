...
/*b. продукты, которые содержат в своем имени слово word*
    ...
    public List<Product> findProductsByName(String word) {
        try (Connection connection = getConnection();
             PreparedStatement prepareStatement = connection.prepareStatement("SELECT * FROM PRODUCTM WHERE NAME  LIKE ? ")) {
...
//                    throw new Exception("User with id: " + validation + " already exist");
//              else if (!validation.matches("[a-z]+"))yfg
//                  throw new Exception("User with id: " + validation + " already exist");

//                String str="%" + word + "%";  /^123abc$/  ^*toy*{2,}$
                if(validation.matches("toy"))
